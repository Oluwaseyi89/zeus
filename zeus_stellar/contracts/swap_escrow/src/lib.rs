#![no_std]
use soroban_sdk::{
    contract, contractimpl, contracttype, token, Address, Bytes, BytesN, Env, IntoVal, Val
};

// Import client components generated from your interfaces crate
use zeus_interfaces::ZkAtomicSwapVerifierClient;
...
impl SwapEscrowContract {
    ...
    pub fn initialize(
        ...
        env.storage().instance().set(&DataKey::SwapAmount, &swap_amount);
    ...
    pub fn claim_swap(
        ...
        let is_valid = verifier_client.verify_btc_swap(&tx_hash, &seal, &journal);
        if !is_valid {
            panic!("Invalid zero-knowledge proof verification failed");
        }
