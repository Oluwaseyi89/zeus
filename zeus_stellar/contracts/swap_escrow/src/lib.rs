...
impl SwapEscrowContract {
    ...
    pub fn claim_swap(
        ...
        // Ensure the caller address matches the target recipient committed to the private off-chain proof
        if verified_journal.recipient_stellar != recipient {
            panic!("Recipient address mismatch with verified journal");
        }

        // Ensure the configured swap amount matches the cryptographic asset proof intent
        if verified_journal.swap_amount != expected_amount as u128 {
            panic!("Swap amount mismatch with verified journal");
        }

        // 5. Dispatch the pre-funded native Stellar assets directly to the verified recipient
        let token_client = token::Client::new(&env, &token_addr);
