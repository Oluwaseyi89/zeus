...
export type StorageKey =
  ...
  | { tag: 'RouterId'; values: void }
  | { tag: 'Active'; values: void }
  | { tag: 'Initialized'; values: void }
  | { tag: 'Relayer'; values: readonly [string] };
...
export interface Client {
   ...
   */
  initialize: (
    { admin, router_id }: { admin: string; router_id: string },
   ...
   */
  is_tx_spent: (
    { btc_tx_hash }: { btc_tx_hash: Buffer },
    options?: MethodOptions,
  ...
  verify_btc_swap: (
    {
      ...
      seal,
      image_id,
    }: { journal_bytes: Buffer; seal: Buffer; image_id: Buffer },
    options?: MethodOptions,
  ) => Promise<AssembledTransaction<BtcSwapJournal>>;
   ...
   */
  update_router_id: (
    { new_router_id }: { new_router_id: string },
    options?: MethodOptions,
  ) => Promise<AssembledTransaction<null>>;
...
export class Client extends ContractClient {
  ...
  constructor(public readonly options: ContractClientOptions) {
    super(
      new ContractSpec([
        'AAAAAgAAAAAAAAAAAAAAClN0b3JhZ2VLZXkAAAAAAAUAAAAAAAAAAAAAAAVBZG1pbgAAAAAAAAAAAAAAAAAACFJvdXRlcklkAAAAAAAAAAAAAAAGQWN0aXZlAAAAAAAAAAAAAAAAAAtJbml0aWFsaXplZAAAAAABAAAAAAAAAAdSZWxheWVyAAAAAAEAAAAT',
